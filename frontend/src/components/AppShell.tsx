...
const ACTIVE = 'flex items-center gap-space-sm px-gutter py-space-sm rounded transition-colors bg-[#1677ff] text-on-primary font-medium'
const IDLE =
  'flex items-center gap-space-sm px-gutter py-space-sm rounded text-[rgba(255,255,255,0.75)] hover:bg-[rgba(255,255,255,0.08)] hover:text-on-primary transition-colors'
...
export function AppShell() {
  ...
  return (
    <CrumbsContext.Provider value={setCrumbs}>
      <aside className="fixed left-0 top-0 h-screen w-60 bg-[#001529] z-50 flex flex-col justify-between select-none">
        <div className="flex flex-col">
          <Link className="h-16 flex items-center gap-space-sm px-gutter-lg bg-[#002140]" to="/">
            <Logo className="h-8 w-8" />
            <span className="font-card-title text-card-title text-[#ffffff] tracking-tight whitespace-nowrap">IT Asset Manager</span>
          </Link>
          <nav className="flex flex-col gap-space-xs px-space-sm pt-space-md">
            {NAV.filter((item) => isAdmin || !item.adminOnly).map((item) => (
              <NavLink key={item.to} className={({ isActive }) => (isActive ? ACTIVE : IDLE)} end={item.end} to={item.to}>
                ...
                <span className="font-body-medium text-body-medium">{item.label}</span>
              </NavLink>
            ))}
          </nav>
        </div>
        <div className="px-gutter-lg py-space-md border-t border-[rgba(255,255,255,0.1)]">
      ...
      <div className="pl-60 min-h-screen flex flex-col">
        <header className="sticky top-0 h-16 bg-surface-container-lowest border-b border-[#f0f0f0] z-40 px-gutter-lg flex items-center justify-between shadow-[0_1px_4px_rgba(0,0,0,0.02)]">
          ...
          {user && (
            <div className="flex items-center gap-gutter">
              <div className="flex items-center gap-space-sm">
                ...
                <span className="font-body-medium text-body-medium text-on-surface">{user.fullName}</span>
              ...
              <button className="flex items-center gap-space-xs text-on-surface-variant hover:text-error transition-colors" onClick={logout} type="button">
                ...
                <span className="font-body-medium text-body-medium">Logout</span>
              </button>
            </div>
          )}
        </header>
        <main className="flex-1 w-full bg-[#f5f7fa] p-gutter-lg">
          <Outlet />
        </main>
      </div>
